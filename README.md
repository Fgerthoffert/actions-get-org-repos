  ...
  get-org-repos:
    ...
    steps:
      - name: Create Release Notes
        uses: fgerthoffert/actions-get-org-repos@main # Replace main by the release of your choice
        with:
          org: zencrepes
          token: YOUR_TOKEN
```
  ...
  get-org-repos:
    ...
    steps:
      ...
      - name: Convert the NDJSON to CSV
        shell: bash
        run: |
          npx @json2csv/cli -i ${{ steps.get-repos.outputs.artifact_filepath }} --ndjson --config json2csv-config.js -o repositories.csv
